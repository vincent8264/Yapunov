# Technical guide: Private Local Sound Alerts

Commands in this guide run from the repository root. The
[README](../README.md) gives the short project overview and quick start.
Multiline shell commands use Bash continuation syntax; in PowerShell, put the
command on one line or replace each trailing backslash with a backtick.
...
and smoke-alarm download locations and licenses. The documented `help-kws.onnx`
build is a locally trained YAMNet-transfer prototype. Its held-out real-speech
result is not good enough for emergency use; review the recorded false-positive
and false-negative counts before experimenting with it.
...
To validate USB reconnection on a physical board, start with the live spectrum
visible, unplug only the USB microphone, and leave the UNO Q and powered hub
connected. Verify the fault icon and warning email, reconnect the microphone,
make a short sound, and verify that spectrum and inference resume before checking
the recovery email. This sequence has not been documented as passing on the board.
...
| `private-live.example.toml` | Template for a private SMTP-enabled configuration, with the optional device heartbeat and its watchdog server. Copy it to ignored `private-live.toml` and never commit credentials. |
...
For board diagnostics, copy the config, select `uno_q`, and add only verified component
...
events. Before relying on live alerts, evaluate it on genuine recordings from the MOVO
...
The repository supplies and tests the scheduler and ONNX adapter. It also documents
how to build a local `help-kws.onnx` prototype, but does not commit that binary.
Its held-out performance is too weak for emergency use; see
[the recorded evaluation](../models/README.md#local-help-keyword-prototype).
Before experimenting with the example config, build the model with the documented
contract, then measure latency on the UNO Q and tune `environment_every_steps` if inference falls
...
To check SMTP settings, send one test email without starting the microphone, model, or
hardware:
...
configured JSON file with owner-only POSIX permissions on the UNO Q. Windows file
permissions depend on its ACLs and are not covered by the POSIX mode tests. The SMTP
password stays in the environment, and a manual portal stops when you press Ctrl+C. The regular `edge-ai
...
The device-side email alerts cannot report a board that has lost power, lost its
network, or crashed. For that, the board sends a small heartbeat to a separate
computer, and that computer emails the configured recipients when heartbeats stop.

While the main loop is running, the board POSTs `{"device_id": ...}` with a
bearer token to the configured `url` every `interval_seconds` (60 s by default).
If the loop has not advanced for `stall_seconds`, it stops sending, so a stalled
app is also detected. Heartbeat failures are logged once and never affect local
alerts. A microphone fault does not stop heartbeats; it has its own email.

The watchdog emails once when it receives no heartbeat for
`missing_after_seconds`. The default is 150 s, so one lost request does not
trigger a warning. It sends one more email when heartbeats resume. Its timer
starts when the server starts, so a board that never comes up is also reported.
...
uv run edge-ai heartbeat-server --config configs/private-live.toml
```

Keep the watchdog running on a computer that stays on and is reachable from the
UNO Q. The packager copies only `[heartbeat]` into the board config, writes the
token to `python/heartbeat-token` inside the private ZIP, and rejects a
`localhost` URL. Run `edge-ai run --config configs/private-live.toml` on the
laptop to test both sides locally first; for that laptop-only test, point `url`
at `127.0.0.1` and package afterward with the LAN address.

Limitations: heartbeats use plain HTTP on the local network, so another device
on that network could read the token. The watchdog cannot tell a dead board from
a network outage between the board and the server. The watchdog keeps no state,
so restarting it resets its timer. A warning is also not sent if the server itself
cannot reach SMTP. Board-to-laptop delivery in App Lab Network Mode still needs to
be verified on the physical UNO Q.
