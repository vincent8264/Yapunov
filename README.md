...
| `audio-caption.example.toml` | Laptop-only optional Whisper audio-caption evaluation; it requires a locally downloaded checkpoint and never targets the UNO Q. |
| `audio-caption-live.example.toml` | Laptop-only live-microphone Whisper evaluation using a five-second rolling audio buffer; it never drives alerts or notifications. |
...
## Laptop audio-caption experiment

This optional workflow evaluates MU-NLPC Whisper Small Audio Captioning on local
WAV files. It is deliberately laptop-only: generated captions are not calibrated
event confidences and never drive UNO Q alerts or notifications.

Install the optional local dependencies and download the checkpoint once:

```bash
uv sync --extra audio-caption
uv run --extra audio-caption hf download MU-NLPC/whisper-small-audio-captioning \
  --local-dir models/whisper-small-audio-captioning
```

Caption one or more PCM WAV recordings. `clotho` produces readable scene
descriptions; `audioset` produces keywords that can be easier to compare with the
existing event taxonomy.

```bash
uv run --extra audio-caption edge-ai caption-audio runs/recordings/example.wav \
  --model-dir models/whisper-small-audio-captioning --style clotho
```

For the reusable pipeline version, copy `configs/audio-caption.example.toml` to
an ignored local file, set its WAV and model paths, and run exactly one step. The
config uses a complete resampled clip up to the model's 30-second input maximum;
it does not reuse the live one-second YAMNet window.

For live listening, copy `configs/audio-caption-live.example.toml` to the ignored
`configs/audio-caption-live.toml`, select a verified microphone name or index if
needed, then run:

```bash
uv run --extra audio-caption edge-ai run --config configs/audio-caption-live.toml
```

It buffers five one-second microphone frames before the first caption, then emits a
new five-second window after every further five seconds. Caption generation happens
in the same loop as capture, so this is a quality-evaluation listener rather than a
lossless, continuous recorder; it may miss audio while the model is generating text.
